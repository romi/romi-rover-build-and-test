...
class RomiOdrive {
	...
	public:
		...
		void moveToAcc(float posInTurns);
		void moveAt(float speed);
		void moveAtNorm(float speed);
		void moveAttAcc(float velocity, float ramp_rate=5); // ramp_rate is in turns/sec
		void moveAttAccNorm(float velocity, float ramp_rate=5); 	// Normalized velocity (-1 - 1)
