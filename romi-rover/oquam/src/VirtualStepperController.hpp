 ...
 */
#ifndef _OQUAM_VIRTUALSTEPPERCONTROLLER_HPP_
#define _OQUAM_VIRTUALSTEPPERCONTROLLER_HPP_

#include "StepperController.hpp" 

class VirtualStepperController : public StepperController
{
public:
        
        VirtualStepperController(double *xmax, double *vmax, double *amax,
                                 double deviation, double *scale, double interval)
                : StepperController(xmax, vmax, amax, deviation, scale, interval),
                  _delay(0) {
                for (int i = 0; i < 3; i++)
                        _stepper_position[i] = 0;
        }
        
        virtual ~VirtualStepperController() {};
...
#endif // _OQUAM_VIRTUALSTEPPERCONTROLLER_HPP_
