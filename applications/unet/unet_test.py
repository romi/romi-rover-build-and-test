...
def handle_unet_test(params):
    folder = os.path.dirname("./")
    image_path = params["path"]
    output_name = params["output-name"]
    img = cv2.imread("./test.png")
    cv2.imwrite(folder + "/" + output_name + ".png", img)
    time.sleep(0.5)
    print(f"New request, image: {image_path}, output name: {output_name}")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--registry', type=str, nargs='?', default="192.168.0.179",
                    help='Set the IP address of the registry')
    parser.add_argument('--ip', type=str, nargs='?', default="192.168.0.179",
                    help='The local IP address to use')
    parser.add_argument('--test', type=bool, nargs='?', default=False,
                        help='The local IP address to use')
    ...
    server = Server("python",
                    {
                        "unet": handle_unet_test,
                        "svm": handle_unet_test
                    },
